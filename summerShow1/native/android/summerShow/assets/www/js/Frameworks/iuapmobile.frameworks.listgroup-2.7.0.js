	...
	paths : {
		...
		if ( typeof module === "object" && typeof module.exports === "object") {
			module.exports = global.document ? factory(global, true) : function(w) {
				if (!w.document) {
					throw new Error("ListGroup requires a window with a document");
				}
				return factory(w);
			};
		...
		function ListGroup(selector, options) {
		...
		ListGroup.prototype.init = function(options) {
			var self = this;
			...
			self.opts = $.extend({}, {
				...
				width : 0,
				domUp : {
					domClass : 'um-listgroup-up',
					domRefresh : '<div class="um-listgroup-refresh">↓下拉刷新</div>',
					domUpdate : '<div class="um-listgroup-update">↑释放更新</div>',
					domLoad : '<div class="um-listgroup-load"><span class="pullLoading"></span>加载中...</div>'
				},
				domDown : {
					domClass : 'um-listgroup-down',
					domRefresh : '<div class="um-listgroup-refresh">↑上拉加载更多</div>',
					domUpdate : '<div class="um-listgroup-update">↓释放加载</div>',
					domLoad : '<div class="um-listgroup-load"><span class="pullLoading"></span>加载中...</div>'
				},
				hasItemMenu : true,
			...
			if (this.opts.collapsible) {
				this.$element.delegate('.um-listgroup-title', 'click', function() {
					var $elem = $(this);
					var $parent = $elem.parent();
					var $sibling = $elem.siblings('ul');

					if ($parent.hasClass('um-listgroup-collapsed')) {
						$parent.removeClass('um-listgroup-collapsed');
						$sibling.slideDown(500);
					} else {

						$parent.addClass('um-listgroup-collapsed');
						$sibling.slideUp(500);
					}
				});
			}
			...
			self.$list.on('touchstart', '.um-listgroup-row', function(e) {
			...
			self.$list.on('touchmove', '.um-listgroup-row', function(e) {
				if (!self.config.itemSwipeEnable)
					return;
				...
				switch (touchDirection) {
					case 'left':
						...
						!self.config.hasOpeningItemMenu && self._events.trigger('itemSwipeLeft', self, $elem);
						break;
					case 'right':
						e.preventDefault();
						e.stopPropagation();
						self.config.isScroll = false;
						!self.config.hasOpeningItemMenu && self._events.trigger('itemSwipeRight', self, $elem);
						break;
					default:
						self.config.isScroll = true;
				}
			...
			self.$list.on('touchend', '.um-listgroup-row', function(e) {
			...
			self.resizeFn = function() {
				if (self.opts.height > 0) {
					...
					self.$parent.addClass('um-listgroup-adaptation');
				}
		...
		ListGroup.prototype.refresh = function() {
			this._continuousPullTimes = 0;
			this.hideLoadingBar();
		};

		ListGroup.prototype.hideLoadingBar = function() {
			if (!this._$domResult)
				return false;
		...
		ListGroup.prototype.on = function(event, callback) {
		...
		ListGroup.prototype.pullDown = function(callback) {
		...
		ListGroup.prototype.pullUp = function(callback) {
			if ( typeof callback == 'function') {
		...
		ListGroup.prototype.tapHold = function(callback) {
			if ( typeof callback == 'function') {
		...
		ListGroup.prototype.itemClick = function(callback) {
			var self = this;
			if ( typeof callback == 'function') {
				this._events.on("itemClick", callback);
				if (!this.config.itemClickEnable) {
					this.$list.delegate('.um-listgroup-row', 'click', function(e) {
						window.event.preventDefault();
						var $rows = self.$list.find('.um-listgroup-row');
						var $target = $(this);
						var $swipeBtns = $target.find(".um-swipe-btns");
						var groupIndex = $target.closest('.um-listgroup-group').index();
		...
		ListGroup.prototype.itemDelete = function(callback) {
			var self = this;
			if ( typeof callback == 'function') {
				...
				if (!this.config.itemDeleteEnable) {
					this.$list.delegate('.um-listgroup-row ' + '.um-delete', 'click', function(e) {
						e.stopPropagation();
						e.preventDefault();
						var $target = $(this).closest('li');
						var $rows = self.$list.find('.um-listgroup-row');
						var groupIndex = $target.closest('.um-listgroup-group').index();
						var rowIndex = $rows.index($target[0]);
						var childIndex = $target.index();
						var args = {
							groupIndex : groupIndex,
							childIndex : childIndex,
		...
		ListGroup.prototype.itemSwipeLeft = function(callback) {
			var self = this;
			if ( typeof callback == 'function') {
				this.config.itemSwipeEnable = true;
				this._events.on('itemSwipeLeft', function(sender, args) {
					var $target = args;
					var $rows = self.$list.find('.um-listgroup-row');
					var groupIndex = $target.closest('.um-listgroup-group').index();
					var childIndex = $target.index();
		...
		ListGroup.prototype.itemSwipeRight = function(callback) {
			var self = this;
			if ( typeof callback == 'function') {
				this.config.itemSwipeEnable = true;
				this._events.on('itemSwipeRight', function(sender, args) {
					var $target = args;
					var $rows = self.$list.find('.um-listgroup-row');
					var groupIndex = $target.closest('.um-listgroup-group').index();
					var childIndex = $target.index();
		...
		ListGroup.prototype.showItemMenu = function($elem) {
		...
		ListGroup.prototype.hideItemMenu = function($elem) {
			if ( typeof $elem !== 'object')
				return false;
			var $domSideMenu = $elem.find('.um-swipe-btns');
			$domSideMenu.removeClass('um-swipe-open');
		...
		window.UM.ListGroup = ListGroup;
		window.UM.listgroup = function(selector, opts) {
			return new ListGroup(selector, opts);
		};
	}));
